...
def get_lexinf_artifacts(config):
    ...
    # end-task model
    model = MLPClassifier(config, len(label_field.vocab), args_field.vocab, args_field.vocab, config.d_rels*3 )
    model.cuda()
...
def main(config):
    ...
    evaluate(train_iterator, dev_iterator,test_iterator,  model, config, opt, label_field, relation_embedding_model, glove)
...
def get_glove_concat_embedding(word1, word2, glove, relation_embedding_model):
    ...
    pair_embedding = torch.cat((word1_embedding, word2_embedding, word1_embedding * word2_embedding), dim=-1)
...
def get_word_relation_embedding(word1, word2, glove, relation_embedding_model):
    relation_embedding = get_relation_embedding(word1, word2, glove, relation_embedding_model)
    word1_embedding = relation_embedding_model.represent_arguments(word1).detach()
    word2_embedding =  relation_embedding_model.represent_arguments(word2).detach()
    return torch.cat((relation_embedding, word1_embedding, word2_embedding), -1)

def get_all_pairs(word3, relation_embedding_model):
    # v, d
    candidate_embeddings = relation_embedding_model.represent_arguments.weight.data
    vocab_size, dim = candidate_embeddings.size()
    bs, dim = word3.size()
    word3 = word3.repeat(vocab_size, 1)
    candidate_embeddings = candidate_embeddings.repeat(bs, 1)
    return word3, candidate_embeddings


def evaluate(iterator, model, config, relation_embedding_model, glove):
    logger.info(model)
    correct = []
    total = 0.0
    for batch_index, batch in enumerate(iterator):
        total += 1
        word1, word2, word3, word4 = batch
        w1_embed, w2_embed, w3_embed = relation_embedding_model.represent_arguments(word1), relation_embedding_model.represent_arguments(word2), relation_embedding_model.represent_arguments(word3)
        w1w2_relations = relation_embedding_model.predict_relations(w1_embed, w2_embed)
        w3_embed, candidate_embed  = get_all_pairs(w3_embed, relation_embedding_model)
        w3c_relations = relation_embedding_model.predict_relations(w3_embed, candidate_embed)
        bs, dim = w1_embed.size()
        scores = (w1w2_relations.repeat(vocab_size, 1) * w3c_relations).sum(-1).view(bs, vocab_size)
        _, predicted_w4 = torch.argmax(scores, -1)
        correct.extend((torch.eq(word4, predicted_w4).float() * torch.neq(word4, 0).float()).cpu().numpy().tolist())
...
def train(train_iterator, dev_iterator, test_iterator, model, config, opt, label_field, relation_embedding_model, glove):
    ...
    composition_fn = get_word_relation_embedding
