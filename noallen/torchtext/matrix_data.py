...
def sample(instances, alpha=None, compositional_rels=True, type_scores=None, type_indices=None):
    # if alpha is None:
...
class TripletIterator():
    def __init__(self, batch_size, fields, return_nl=False, limit=None, compositional_rels=True, type_scores_file=None, type_indices_file=None):
    ...
    def _create_batches(self, instance_gen, device=-1, train=True):
        for instances in instance_gen:
            ...
            inputs = instances if not train else sample(instances, 0.75, self.compositional_rels, self.type_scores, self.type_indices)
            # import ipdb
            # ipdb.set_trace()
...
def read_dev(fname, limit=None, compositional_rels=True, type_scores_file=None, type_indices_file=None):
    ...
    return sample(instances, alpha=.75, compositional_rels=compositional_rels, type_scores=type_scores, type_indices=type_indices)
...
def create_dataset(config):
    ...
    validation_sample = read_dev(files[0], 500000, config.compositional_rels, type_scores_file, type_indices_file)
...
def read_data(config, return_nl=False, preindex=True):
    ...
    train_iterator = TripletIterator(config.train_batch_size, fields , return_nl=return_nl, 
            compositional_rels=config.compositional_rels, type_scores_file=type_scores_file, type_indices_file=type_indices_file)
    dev_iterator = TripletIterator(config.dev_batch_size, fields, return_nl=return_nl, compositional_rels=config.compositional_rels)
