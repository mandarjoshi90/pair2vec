...
class DatasetReader(Registrable):
    ...
    def read(self, params: Params) -> Iterable[Instance]:
        """
        ...
        if lazy:
            return _LazyInstances(lambda: iter(self._read(params)))
        else:
            instances = self._read(params)
            if not isinstance(instances, list):
                instances = [instance for instance in Tqdm.tqdm(instances)]
            if not instances:
                raise ConfigurationError("No instances were read from the given filepath {}. "
                                         "Is the path correct?".format(params))
            return instances

    def _read(self, params: Params) -> Iterable[Instance]:
        """
