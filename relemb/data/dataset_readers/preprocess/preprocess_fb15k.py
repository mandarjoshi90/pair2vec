...
def create_relation_candidates(triples, top_k=2):
    ...
    relation_to_candidate_subjects, relation_to_candidate_objects = defaultdict(set), defaultdict(set)
    for relation in relation_to_subject_type:
        for subject_type, _ in relation_to_subject_type[relation]:
    ...
    for relation in relation_to_object_type:
        for object_type, _ in relation_to_object_type[relation]:
...
def build_graph(triples, subject_relation_to_objects, object_relation_to_subjects, subject_relation_to_objects_per_file, object_relation_to_subjects_per_file):
    for e1, relation, e2 in triples:
        subject_relation_to_objects[(e1, relation)].add(e2)
        object_relation_to_subjects[(e2, relation)].add(e1)

        subject_relation_to_objects_per_file[(e1, relation)].add(e2)
        object_relation_to_subjects_per_file[(e2, relation)].add(e1)
...
def preprocess(data_dir, files=('train.txt', 'valid.txt', 'test.txt'), preprocessed_dir='preprocessed'):
    ...
    for file in files:
        ...
        subject_relation_to_objects_per_file, object_relation_to_subjects_per_file = defaultdict(set), defaultdict(set)
        build_graph(triples, subject_relation_to_objects, object_relation_to_subjects,
                    subject_relation_to_objects_per_file, object_relation_to_subjects_per_file)

        logger.info('Writing for {}'.format(file))
        write_to_file(os.path.join(preprocess_dir, 'subject_relation_to_objects_{}.tsv'.format(file)), subject_relation_to_objects)
        write_to_file(os.path.join(preprocess_dir, 'object_relation_to_subjects_{}.tsv'.format(file)), object_relation_to_subjects)
