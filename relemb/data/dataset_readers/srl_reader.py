...
from allennlp.data.fields import Field, TextField, SequenceLabelField, IndexField, ListField
...
@DatasetReader.register("srl_for_relemb")
class RelembSrlReader(DatasetReader):
    """
    ...
    def text_to_instance(self,  # type: ignore
        ...
        verb_idxs = [i for (i, is_verb) in enumerate(verb_label) if is_verb == 1]
        if len(verb_idxs) > 0:
            fields['verb_idxs'] = ListField([IndexField(idx, text_field) for idx in verb_idxs])
        else:
            fields['verb_idxs'] = ListField([IndexField(-1, text_field)])
        if tags:
    ...
    def from_params(cls, params: Params) -> 'SrlReader':
        ...
        return RelembSrlReader(token_indexers=token_indexers,
