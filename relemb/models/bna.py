...
@Model.register("bidaf-no-answer")
class BidafNoAnswer(Model):
    """
    ...
    def get_span_scores(span_start_logits, span_end_logits):
        ...
        return torch.exp(add_se_scores) #* upper_triu.unsqueeze(0)i
    ...
    def _get_best_span(self, span_start_logits: Variable, span_end_logits: Variable) -> Variable:
        ...
        for b in range(batch_size):  # pylint: disable=invalid-name
            for j in range(passage_length):
                ...
                if val1 + val2 > max_span_log_prob[b] and (j - span_start_argmax[b]) <= 17:
