...
from relemb.modules.input_variational_dropout import InputVariationalDropout
...
@Model.register("relemb_dialog_qa")
class RelembDialogQA(Model):
    """
    ...
    def __init__(self, vocab: Vocabulary,
        ...
        max_turn_length = 12
        self.relemb_model_file = model_file
        ...
        load_model(model_file, self.relemb)
        ...
        self._merge_atten = TimeDistributed(torch.nn.Linear(self._encoding_dim * 4 + 600, self._encoding_dim))
    ...
    def forward(self,  # type: ignore
        ...
        passage_len = repeated_encoded_passage.size(1)
        question_len = encoded_question.size(1)
        if self._num_context_answers > 0:
            relemb_passage_tokens = relemb_passage_tokens.unsqueeze(1).repeat(1, max_qa_count, 1).contiguous().view(-1, passage_len)
        relemb_question_tokens = relemb_question_tokens.contiguous().view(-1, question_len)
        # import ipdb
        # ipdb.set_trace()
