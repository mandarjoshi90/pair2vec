...
@SimilarityFunction.register("hadamard_mlp")
class HadamardSimilarity(SimilarityFunction):
    """
    ...
    def __init__(self,
        ...
        super(HadamardSimilarity, self).__init__()
        combined_dim = tensor_dim + 1
    ...
    @overrides
    def forward(self, tensor_1: torch.Tensor, tensor_2: torch.Tensor, t1_dot_t2: torch.Tensor) -> torch.Tensor:
        combined_tensors = torch.cat((tensor_1 * tensor_2, t1_dot_t2), dim=-1)
        dot_product = torch.matmul(combined_tensors, self._weight_vector)
        return self._activation(dot_product + self._bias)

    @classmethod
    def from_params(cls, params: Params) -> 'HadamardSimilarity':
        tensor_dim = params.pop_int("tensor_dim")
        activation = Activation.by_name(params.pop("activation", "linear"))()
        params.assert_empty(cls.__name__)
        return cls(tensor_dim=tensor_dim,
